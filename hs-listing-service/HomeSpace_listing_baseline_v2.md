...
Dropdown dùng chung ở Căn hộ, Nhà nguyên căn và Phòng. Mapping hai chiều 1:1, không còn giá trị nào rơi vào fallback:

| Nhãn frontend | Giá trị frontend | Enum backend |
|---|---|---|
| Bàn giao thô / Chưa có nội thất | `RAW` | `UNFURNISHED` |
| Nội thất cơ bản | `BASIC` | `BASIC` |
| Nội thất một phần | `PARTIAL` | `PARTIALLY_FURNISHED` |
| Đầy đủ nội thất | `FULL` | `FULLY_FURNISHED` |
| Nội thất cao cấp | `LUXURY` | `LUXURY` |

Tạo payload dùng `resolveFurnishing()`, nạp tin để sửa dùng `furnishingStatusToFormValue()`; cả hai nằm cạnh nhau trong `buildListingPayload.ts` và phải sửa cùng nhau.

Giá trị này quyết định bảng trang thiết bị bàn giao ở mục 3.7 có bắt buộc hay không.

### 3.7. Bảng trang thiết bị bàn giao

Áp dụng cho **cả 5 loại hình**, nằm trong section 2 (Thông tin chi tiết), ngay dưới dropdown Tình trạng nội thất / Tình trạng bàn giao.

Mỗi dòng gồm bốn cột hiển thị đúng thứ tự bảng `{{#equipmentTable}}` của hợp đồng:

| Cột UI | Field payload | Bắt buộc |
|---|---|---|
| STT | không gửi, backend tự đánh theo `sortOrder` | — |
| Tên tài sản / Trang thiết bị | `assetName` | Có |
| Số lượng | `quantity`, số nguyên >= 1 | Có |
| Hiện trạng bàn giao | `handoverCondition` | Có |
| Ghi chú | `conditionNote`, tối đa 255 ký tự | Không |

Cách nhập: bấm chip trong danh mục động (mục 6.2) để sinh nhanh một dòng với `itemCode` và tên mặc định từ catalog; tên vẫn sửa được để ghi cụ thể model/nhãn hiệu. Nút "Thêm tài sản khác" tạo dòng nhập tay với `itemCode = null`.

Dropdown Hiện trạng bàn giao — enum backend `HandoverCondition`:
...
Quy tắc bắt buộc có điều kiện, enforce cả frontend (`validateAllFields`) lẫn backend (`ListingService.requiresFurnishingInventory`):

| Category | Trường điều kiện | Bắt buộc >= 1 dòng khi |
|---|---|---|
| `APARTMENT`, `HOUSE`, `ROOM` | `furnishingStatus` | khác `UNFURNISHED` |
| `OFFICE` | `officeDetail.handoverStatus` | khác `RAW` |
| `COMMERCIAL_SPACE` | `commercialDetail.handoverStatus` | khác `RAW` |

Bàn giao thô được phép để trống bảng. Backend trả `422 furnishings [REQUIRED]` khi thiếu, `422 furnishings.itemCode [INVALID_FOR_CATEGORY]` khi chip không thuộc category, và `404 FURNISHING_NOT_FOUND` khi code không có trong catalog.

Response đọc về là `ListingFurnishingResponse` đã đánh sẵn `index` liên tục 1..n và `conditionText` dạng `"Còn tốt — làm lạnh nhanh"`, map thẳng vào `{index, name, quantity, condition}` của `ContractRenderService.buildEquipmentTable()`.
...
- Bảng trang thiết bị bàn giao: mục 3.7.
...
Dropdown Tình trạng bàn giao — enum backend `OfficeHandoverStatus`, giá trị frontend và API trùng nhau:
...
Dropdown Tình trạng bàn giao — enum backend `CommercialHandoverStatus`, giá trị frontend và API trùng nhau:
...
Response gồm `amenities` và `furnishings`, **cả hai đều lọc theo category** qua bảng map riêng (`amenity_categories`, `furnishing_item_categories`). Mỗi item gồm `code`, `name`, `sortOrder`.
...
### 6.2. Nội thất/trang thiết bị theo category

Catalog dùng chung, không tạo lại mỗi lần đăng tin; đây chỉ là danh sách chip gợi ý để sinh nhanh dòng trong bảng ở mục 3.7. Người dùng luôn có thể sửa tên hoặc thêm tài sản ngoài catalog.

#### APARTMENT

`BED`, `WARDROBE`, `WORK_DESK`, `KITCHEN_SHELF`, `REFRIGERATOR`, `WASHING_MACHINE`, `WATER_HEATER`, `CURTAIN`, `FAN`, `SOFA_SET`, `DINING_SET`, `TV`, `KITCHEN_CABINET`, `COOKTOP`, `RANGE_HOOD`, `MICROWAVE`, `SHOE_CABINET`, `AIR_CONDITIONER`, `LIGHTING`.

#### HOUSE

Toàn bộ danh sách của APARTMENT, cộng thêm `WATER_TANK_PUMP` — Bồn nước / máy bơm.

#### ROOM

`BED`, `WARDROBE`, `WORK_DESK`, `KITCHEN_SHELF`, `REFRIGERATOR`, `WASHING_MACHINE`, `WATER_HEATER`, `CURTAIN`, `FAN`, `AIR_CONDITIONER`, `LIGHTING`.

#### OFFICE

`OFFICE_DESK`, `OFFICE_CHAIR`, `FILING_CABINET`, `PARTITION`, `MEETING_TABLE`, `RECEPTION_COUNTER`, `NETWORK_CABLING`, `PROJECTOR_TV`, `PANTRY_EQUIPMENT`, `CURTAIN_BLIND`, `AIR_CONDITIONER`, `LIGHTING`, `CEILING_FLOOR_FINISH`, `FIRE_EQUIPMENT`.

#### COMMERCIAL_SPACE

`ROLLING_DOOR`, `DISPLAY_SHELF`, `CASHIER_COUNTER`, `SIGNAGE_FRAME`, `ELECTRICAL_SYSTEM`, `WATER_SYSTEM`, `RESTROOM_FIXTURE`, `MEZZANINE_STRUCTURE`, `WAREHOUSE_SHELF`, `AIR_CONDITIONER`, `LIGHTING`, `CEILING_FLOOR_FINISH`, `FIRE_EQUIPMENT`.

Với OFFICE và COMMERCIAL_SPACE, bảng này là **biên bản kiểm kê hiện trạng bàn giao** theo đúng yêu cầu của `CONTRACT_PROMPT.md`, nên catalog gồm cả hạng mục kết cấu và hệ thống (trần/sàn hoàn thiện, hệ thống điện, PCCC, cửa cuốn), không chỉ đồ nội thất rời.

`code` là UNIQUE toàn bảng, nên item dùng ở nhiều loại hình (ví dụ `AIR_CONDITIONER`) chỉ có một row với `categories` là tập hợp. Seeder `ListingReferenceDataInitializer.furnishing()` chạy theo cơ chế upsert để backfill `categories` cho item đã seed từ trước.
...
- `furnishing_item_categories`: category áp dụng cho từng item catalog.
- `listing_furnishing_assets`: bảng kiểm kê trang thiết bị bàn giao của từng listing; cột `furnishing_item_id` (nullable), `item_code`, `asset_name`, `quantity`, `handover_condition`, `condition_note`, `sort_order`.
- `listing_furnishings`: bảng nối many-to-many cũ, **không còn được đọc/ghi**; giữ lại vì `JPA_DDL_AUTO=update` không xóa bảng.
...
`listing_furnishing_assets.asset_name` được denormalize có chủ đích: hợp đồng là văn bản pháp lý nên tên tài sản phải đóng băng tại thời điểm đăng tin và phải ghi được model/nhãn hiệu cụ thể, không phụ thuộc tên hiện tại của catalog.
...
5. ~~Dropdown nội thất map thiếu, `LUXURY` và `RAW` cùng rơi vào `UNFURNISHED`.~~ **Đã xử lý.** Backend thêm `LUXURY`, frontend thêm option `PARTIAL`, và cả hai chiều map đều đầy đủ 1:1 (mục 3.4).
...
20. Dữ liệu nội thất cũ nằm trong bảng nối `listing_furnishings` không được chuyển sang `listing_furnishing_assets`. Các tin tạo trước thay đổi này sẽ hiển thị bảng trang thiết bị rỗng; chủ tin phải kê khai lại khi sửa tin. Nếu môi trường nào cần giữ dữ liệu cũ thì phải viết bước copy riêng (`quantity = 1`, `handover_condition = GOOD`).
