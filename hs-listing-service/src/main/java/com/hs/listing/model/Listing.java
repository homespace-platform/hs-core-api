...
public class Listing extends BaseEntity {
    ...
    @Column(name = "minimum_lease_months", nullable = false)
    private Integer minimumLeaseMonths;
    @Column(name = "management_fee_included", nullable = false)
    ...
    @OneToMany(mappedBy = "listing", cascade = CascadeType.ALL, orphanRemoval = true)
    @Builder.Default
    private List<ListingCharge> charges = new ArrayList<>();
    @OneToMany(mappedBy = "listing", cascade = CascadeType.ALL, orphanRemoval = true)
    @Builder.Default
    private List<ListingCustomAmenity> customAmenities = new ArrayList<>();
    ...
    @OneToMany(mappedBy = "listing", cascade = CascadeType.ALL, orphanRemoval = true)
    @Builder.Default
    private List<ListingFurnishingAsset> furnishings = new ArrayList<>();
    @ElementCollection(fetch = FetchType.LAZY)
    @CollectionTable(name = "listing_viewing_days", joinColumns = @JoinColumn(name = "listing_id"))
    @Column(name = "day_of_week", nullable = false)
    @Enumerated(EnumType.STRING)
    @Builder.Default
    private Set<DayOfWeek> viewingDays = new HashSet<>();
    @ElementCollection(fetch = FetchType.LAZY)
    @CollectionTable(name = "listing_viewing_slots", joinColumns = @JoinColumn(name = "listing_id"))
    @Column(name = "viewing_slot", nullable = false)
    @Enumerated(EnumType.STRING)
    @Builder.Default
    private Set<ViewingSlot> viewingSlots = new HashSet<>();
